  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[linear-gradient(90deg,rgba(4,4,3,0.92),rgba(18,18,15,0.82),rgba(4,4,3,0.92))] backdrop-blur-xl">
      <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" className="group flex items-center gap-3" aria-label="Monkey Scarfs home">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <Link
            href="/cart"
            className="relative grid h-11 w-11 place-items-center rounded-full border border-white/15 bg-white/8 text-white transition hover:border-[#d8ff2f]/60 hover:text-[#d8ff2f]"
            aria-label="Cart"
          >
            <ShoppingBag className="h-4 w-4" />
            {count ? (
              <span className="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-[#d8ff2f] px-1 text-[10px] font-black text-black">
                {count}
              </span>
            ) : null}
          </Link>
